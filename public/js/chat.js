$(function () {
  ...
  let channel = pusher.subscribe('chat');
    ...
    $(".chat-toggle").on("click", function (e) {
     ...
     let id_group = ele.attr("data-id");

     let username = ele.attr("data-user");

     cloneChatBox(id_group, username, function () {

       let chatBox = $("#chat_box_" + id_group);

       if(!chatBox.hasClass("chat-opened")) {

         chatBox.addClass("chat-opened").slideDown("fast");

         loadLatestMessages(chatBox, id_group);
   ...
   channel.bind('send', function(data) {
    displayMessage(data.data,data.user_group);
  });
 ...
 * @param user_id
 */
 function loadLatestMessages(container, id_group)
 {
  ...
  $.ajax({
    ...
    data: {id_group: id_group, _token: $("meta[name='csrf-token']").attr("content")},
    method: "GET",
    dataType: "json",
    beforeSend: function () {
 ...
 */
 function displayMessage(message,user)
 {
  ...
  if($("#current_user").val() == message.from_user_id) {
    ...
    $("#chat_box_" + message.to_group).find(".chat-area").append(messageLine);

  } else {
    for(var k in user) {
        console.log(7);
        // console.log(k, result[k]);
        if($("#current_user").val() != user[k].id_user){
        console.log(user[k].id_user);

          alert_sound.play();

        // for the receiver user check if the chat box is already opened otherwise open it
        cloneChatBox(message.to_group, message.toUserName, function () {

          let chatBox = $("#chat_box_" + message.to_group);

          if(!chatBox.hasClass("chat-opened")) {

            chatBox.addClass("chat-opened").slideDown("fast");

            loadLatestMessages(chatBox, message.to_group);
